...
contract MyOApp is OApp {
    ...
    // Mapping of NFT contract address => tokenId => owner
    ...
    function depositNFT(address nftContract, uint256 tokenId) public {
        ...
        // Store the token's original owner
        tokenOwners[nftContract][tokenId] = msg.sender;
    }

    // Withdraw an NFT from the vault
    function withdrawNFT(address nftContract, uint256 tokenId) public {
        // Check that the msg.sender is the owner who deposited the NFT
        require(tokenOwners[nftContract][tokenId] == msg.sender, "Not the owner");

        // Transfer the NFT back to the original owner
        IERC721(nftContract).safeTransferFrom(address(this), msg.sender, tokenId);

        // Remove the token from the mapping
        delete tokenOwners[nftContract][tokenId];
    }
    ...
    function _lzReceive(
        ...
        // Store the message in the state variable `data`
        data = message;
